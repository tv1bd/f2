...
async def generate_token_with_session(uid, password, session):
    """
    Complete token generation flow using provided session for connection pooling
    """
    try:
        # Step 1: Get access token
        open_id, access_token = await get_access_token_with_session(uid, password, session)
        ...
        response = await perform_major_login_with_session(payload, session)
        if not response:
            return None
        ...
        print(f"❌ Token generation error for UID {uid}: {str(e)[:100]}")
        return None

async def get_access_token_with_session(uid, password, session):
    """
    Step 1: Get access token from Garena OAuth using provided session
    """
    url = "https://100067.connect.garena.com/oauth/guest/token/grant"
    ...
    try:
        ...
        print(f"❌ Access token error for UID {uid}: {str(e)[:50]}")
        return None, None

async def perform_major_login_with_session(payload, session):
    """
    Step 3: Send MajorLogin request to Free Fire server using provided session
    """
    url = "https://loginbp.ggblueshark.com/MajorLogin"
    ...
    try:
        ...
        async with session.post(url, data=payload, headers=headers, ssl=ssl_context, timeout=aiohttp.ClientTimeout(total=20)) as response:
            if response.status == 200:
                return await response.read()
            else:
                return None
    except Exception as e:
        print(f"❌ MajorLogin error: {str(e)[:50]}")
        return None
